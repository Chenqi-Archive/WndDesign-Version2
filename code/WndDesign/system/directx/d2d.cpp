...
ID2D1Bitmap1& Target::GetBitmap() const {
    if (bitmap == nullptr) {
        D2D1_BITMAP_PROPERTIES1 bitmap_properties = D2D1::BitmapProperties1(
            D2D1_BITMAP_OPTIONS_TARGET,
            D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED)
        );
        Size size = layer.GetTileSize();
...
void Target::Clear() {
    SafeRelease(&bitmap);
#error the target might be being drawn.
}

void Target::DrawOn(Vector tile_offset, Rect region, CompositeEffect composite_effect, RenderTarget& target, Vector offset) const {
    if (bitmap == nullptr) {  
        // Draw background.
        // If has other effects, use an independent layer.
        layer.GetBackground().DrawOn(region + tile_offset, composite_effect.opacity, target, offset);
    } else {
        target.DrawBitmap(
            bitmap,
            Rect2RECT(Rect(point_zero + offset, region.size)),
