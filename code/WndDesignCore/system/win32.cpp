...
WNDDESIGNCORE_API std::pair<HANDLE, const Point> ConvertPointToDesktopWndPoint(WndObject& wnd, Point point) {
	return static_cast<DesktopObjectImpl&>(DesktopObject::Get()).ConvertWndNonClientPointToDesktopWindowPoint(wnd, point);
}
...
WNDDESIGNCORE_API HANDLE GetWndHandle(WndObject& wnd) {
	return static_cast<DesktopObjectImpl&>(DesktopObject::Get()).GetWndHandle(wnd);
}
