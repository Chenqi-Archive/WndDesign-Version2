...
//// RedrawQueue and ReflowQueue committing logic ////
...
struct CommitTimer : public Timer {
	static constexpr uint commit_interval = 20; //ms
	virtual void OnAlert() override { should_commit = true; }
	void Set() { Timer::Set(commit_interval); }
} commit_timer;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	// defer CommitQueue();
	struct QueueCommitter { ~QueueCommitter() { CommitQueue(); } } queue_committer;
...
FrameIrrelevantMessages:
	switch (msg) {
	case WM_CREATE: window_cnt++; commit_timer.Set(); break;
	case WM_DESTROY: window_cnt--; if (window_cnt == 0) { commit_timer.Stop(); PostQuitMessage(0); }break;
